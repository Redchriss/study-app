...
**Root cause:** A live-schema audit found 28 operations the server rejects. The biggest broke the **entire Circles feed**: the `PostFields`/`CommentFields` fragment text lived in separate consts (`kPostFields`/`kCommentFields`) that were never concatenated into the consuming queries, so `gql(kHomeFeed)` etc. were sent without the fragment (`Unknown fragment 'PostFields'`). Beyond that, queries requested fields the schema lacks (`isNsfw`, `voteDirection`, `isSaved`, `urlTitle/urlDescription`, `flairEmoji/flairColor`, `isBookmarked`, poll `optionId`), used wrong field names (`communityFlair.backgroundColor`→`color`, community `sidebar`→`sidebarMarkdown`), wrong moderation payload shapes (`{success}` on `PostPayload` mutations; `membership{...} errors` wrapper on `addModerator` which returns `MembershipType`), wrong variable types (`votePost/voteComment $direction: Int!`→`VoteDirectionEnum!`, `resolveReport $action: String!`→`ReportActionEnum!`, scanner `uuid: String!`→`ID!`), wrong diagnostics fields (`conceptSlug`→`concept`, `difficulty` removed, answer `text`→`answerText`), and profile mutations selecting non-existent `id` on `UserProfileType` plus `updateProfileBio` using a `bio:` arg instead of `input: ProfileInput!`.
**Fix:** Concatenated `kPostFields`/`kCommentFields` into all 10 consuming queries; pruned schema-absent fields; renamed mismatched fields/args; switched the four `PostPayload` mutations to read `post`/`errors` (repo call sites updated) and `addModerator` to the bare `MembershipType`; changed vote/report/scanner variable types and mapped vote ints to `UP`/`DOWN` at every call site (repo + 5 widgets); aliased diagnostics fields (`conceptSlug: concept`, `text: answerText`) so the screen parsing is unchanged; fixed the profile mutations and rewrote `updateProfileBio` to use `input: ProfileInput!`. Poll vote-state now reads `userVote.id`.
**Verified:** In-process validation of all operations against the live schema → **196/196 valid, 0 invalid** (resolving the Dart `+ kFragment` concatenations). Live curl confirmed: `homeFeed` with the attached fragment returns data (no "Unknown fragment"), `votePost(direction:"UP")` and `resolveReport(action:"IGNORE")` accept the enum names at runtime. `flutter analyze` on the touched areas → 0 errors/warnings (pre-existing info lints only); `dart format` clean.
