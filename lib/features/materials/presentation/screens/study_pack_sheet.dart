        ...
        child: Row(
          children: [
            Container(
              ...
              child:
                  const Icon(Icons.play_arrow_rounded, color: DesignTokens.accent),
            ),
            const SizedBox(width: DesignTokens.spMd),
            Expanded(
              child: Column(
...
/// generating → ready states with a retry on failure, mirroring the
/// existing material-AI-task polling pattern.
class StudyPackCard extends StatelessWidget {
  ...
  String _subtitle() {
    if (isReady) {
      return 'Lesson, quiz & flashcards are ready to study.';
    }
    if (isGenerating) {
