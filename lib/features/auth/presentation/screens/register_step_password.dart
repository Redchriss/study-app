...
class PasswordStrength {
  ...
  static PasswordStrength evaluate(String password) {
    if (password.isEmpty) return const PasswordStrength(0, '', Colors.transparent);
    int score = 0;
    ...
    if (score <= 1) return const PasswordStrength(1, 'Weak', DesignTokens.error, hint: 'Add uppercase, numbers & symbols');
    if (score <= 3) return const PasswordStrength(2, 'Fair', DesignTokens.warning, hint: 'Mix uppercase, numbers & symbols');
    if (score <= 4) return const PasswordStrength(3, 'Good', DesignTokens.info, hint: 'Almost there — add a symbol');
    return const PasswordStrength(4, 'Strong', DesignTokens.success, hint: 'Your password is secure');
  }
}
...
class _RegisterStepPasswordState extends State<RegisterStepPassword> {
  bool _obscure = true;
  PasswordStrength _strength = const PasswordStrength(0, '', Colors.transparent);
        ...
        child: Column(
          ...
          children: [
            ...
            TextFormField(
              ...
              onChanged: (v) => setState(() => _strength = PasswordStrength.evaluate(v)),
              validator: (v) {
                        ...
                        for (int i = 0; i < 4; i++)
                          Expanded(
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                color: i < _strength.score ? _strength.color : Colors.grey.shade200,
