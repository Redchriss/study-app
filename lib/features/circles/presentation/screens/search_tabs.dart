        ...
        Expanded(
          child: Query(
            ...
            builder: (result, {fetchMore, refetch}) {
              ...
              if (posts.isEmpty) return SearchNoResults(query: query);
...
class SearchCommunitiesTab extends StatelessWidget {
  final String query;
  final bool dark;
  const SearchCommunitiesTab({
    super.key,
    required this.query,
    required this.dark,
  });
  ...
  Widget build(BuildContext context) {
    return Query(
      ...
      builder: (result, {fetchMore, refetch}) {
        ...
        if (communities.isEmpty) return SearchNoResults(query: query);
                    ...
                    children: [
                      CircleAvatar(
                        ...
                        child: icon.isNotEmpty
                            ? null
                            : Text(c['name']?.toString()[0].toUpperCase() ?? 'Y',
  ...
  Widget build(BuildContext context) {
    return Query(
      ...
      builder: (result, {fetchMore, refetch}) {
        ...
        if (users.isEmpty) return SearchNoResults(query: query);
...
class SearchEmptyHint extends StatelessWidget {
  final String? communitySlug;
  const SearchEmptyHint({super.key, this.communitySlug});
